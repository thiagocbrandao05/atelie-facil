...
describe('Lib Coverage', () => {
    ...
    describe('audit', () => {
        it('logAudit should console log (mocked)', async () => {
            ...
            expect(consoleSpy).toHaveBeenCalledWith('[AUDIT]', expect.objectContaining({
                action: 'TEST',
                entity: 'TEST_ENTITY'
            }));
        ...
        it('logAudit should handle errors gracefully', async () => {
            const consoleSpy = vi.spyOn(console, 'log').mockImplementation(() => { throw new Error('Test Error') });
            const consoleErrorSpy = vi.spyOn(console, 'error').mockImplementation(() => { });

            await logAudit({ action: 'TEST', entity: 'TEST', entityId: '1' });

            expect(consoleErrorSpy).toHaveBeenCalledWith('Failed to log audit entry:', expect.any(Error));

            consoleSpy.mockRestore();
            consoleErrorSpy.mockRestore();
        });

        it('withAudit should wrap function and log success', async () => {
            const consoleSpy = vi.spyOn(console, 'log').mockImplementation(() => { });
            const mockFn = vi.fn().mockResolvedValue({ success: true, data: { id: '999' } });

            const wrapped = withAudit('CREATE', 'USER', mockFn);
            const result = await wrapped('arg1');

            expect(result).toEqual({ success: true, data: { id: '999' } });
