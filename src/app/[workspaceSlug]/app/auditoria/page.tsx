...
const AuditLogTable = dynamic(
  () => import('@/components/audit/AuditLogTable').then(mod => ({ default: mod.AuditLogTable })),
  {
    loading: () => <div className="py-8 text-center">Carregando tabela...</div>,
    ssr: true,
  }
)
...
export default async function AuditoriaPage({
  ...
  const [logsData, stats] = await Promise.all([getAuditLogs(page, 50, filters), getAuditStats()])
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Visualize todas as ações realizadas pelos usuários</CardDescription>
        </CardHeader>
        <CardContent>
