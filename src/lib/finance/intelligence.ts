...
export function analyzeProfitability(
    ...
    const breakEvenRevenue = marginPercent.isPositive() ? fixed.dividedBy(marginPercent) : new Decimal(Infinity)
...
/**
 * Scenario Simulation
 * Calculates the impact of a cost increase on profitability.
 */
export function simulateCostImpact(
    currentPrice: number | string | Decimal,
    currentCost: number | string | Decimal,
    costIncreasePercent: number | string | Decimal
): { newMargin: Decimal, marginErosion: Decimal } {
    const price = new Decimal(currentPrice)
    const cost = new Decimal(currentCost)
    const increase = new Decimal(costIncreasePercent).dividedBy(100)

    const newCost = cost.times(new Decimal(1).plus(increase))
    const currentMargin = price.minus(cost)
    const newMargin = price.minus(newCost)

    return {
        newMargin,
        marginErosion: currentMargin.minus(newMargin)
    }
}
