...
import type { ActionResponse, PaginatedResponse, ProductWithMaterials } from '@/lib/types'
...
export async function getProductsPaginated(
    ...
    const formattedData = (data as any[]).map(product => ({
        ...
        materials: product.materials.map((pm: any) => ({
            ...
            material: {
                ...pm.material,
                cost: pm.material.v_material_last_costs?.[0]?.last_cost || pm.material.cost || 0
            }
        }))
    })) as any as ProductWithMaterials[]
...
export async function createProduct(prevState: ActionResponse, formData: FormData): Promise<ActionResponse> {
    ...
    try {
        ...
        if (!materialsJson || materialsJson === '[]') {
            return {
                success: false,
                message: 'Adicione pelo menos um material ao produto.',
            }
        }
        ...
        const { error } = await supabase.rpc('create_product_with_materials', {
        ...
        return { success: false, message: 'Erro ao cadastrar produto: ' + (e.message || 'Erro desconhecido') }
    }
}

export async function updateProduct(id: string, prevState: ActionResponse, formData: FormData): Promise<ActionResponse> {
    const user = await getCurrentUser()
    if (!user) return { success: false, message: 'Não autorizado' }
    ...
    try {
        ...
        const { error } = await supabase.rpc('update_product_with_materials', {
            p_product_id: id,
            p_tenant_id: user.tenantId,
            p_name: validatedFields.data.name,
            p_image_url: validatedFields.data.imageUrl || '',
        ...
        return { success: false, message: 'Erro ao atualizar produto: ' + (e.message || 'Erro desconhecido') }
    }
}
