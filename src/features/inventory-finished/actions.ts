...
import type { ActionResponse, ProductInventory, ProductInventoryMovement, ProductInventoryMovementType } from '@/lib/types'
...
export async function getProductStockAlerts() {
    ...
    return (data as any[])
        ...
        .map(item => ({
            ...
            severity: item.quantity <= 0 ? 'critical' : item.quantity <= (item.minQuantity || 0) / 2 ? 'high' : 'medium'
        }))
}
...
export async function createProductStockEntry(
    ...
    const totalCost = rawData.items.reduce((acc: number, item: any) => acc + (item.quantity * item.unitCost), 0) + rawData.freightCost
