...
export async function generateProductCostAnalysis() {
    ...
    return (products as any).map((product: any) => {
        ...
        const validItems = product.items.filter((item: any) => item.order && item.order.status !== 'CANCELLED')
        const totalSold = validItems.reduce((sum: number, item: any) => sum + Number(item.quantity), 0)
        const totalRevenue = validItems.reduce((sum: number, item: any) => sum + (Number(item.price) * Number(item.quantity)), 0)
