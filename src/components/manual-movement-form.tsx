...
const MOVEMENT_TYPES = [
  {
    ...
    label: 'Ajute de Entrada (+)',
    description: 'Correção de contagem ou bônus',
  },
  ...
  {
    value: 'RETIRADA',
    label: 'Retirada / Uso Interno (-)',
    description: 'Uso não vinculado a pedido',
  },
    ...
    <div className="bg-card rounded-lg border p-6 shadow-sm">
      <div className="mb-6">
        ...
        <p className="text-muted-foreground text-sm">
          Registrar ajustes, perdas ou retiradas avulsas.
        </p>
      </div>
        ...
        <div className="grid gap-6">
          <div className="space-y-3">
            ...
            >
              {MOVEMENT_TYPES.map(type => (
                <div
                  key={type.value}
                  className="has-[:checked]:border-primary has-[:checked]:bg-primary/5 flex items-center space-x-2 rounded-md border p-3"
                >
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <div className="space-y-2">
              ...
              {localState.errors?.materialId && (
                <p className="text-xs text-red-500">{localState.errors.materialId[0]}</p>
              )}
            </div>
            ...
            <div className="space-y-2">
              ...
              <Select name="color" disabled={materialColors.length === 0}>
                <SelectTrigger>
                  <SelectValue
                    placeholder={
                      materialColors.length === 0 ? 'Nenhuma cor cadastrada' : 'Selecione...'
                    }
                  />
                </SelectTrigger>
                <SelectContent>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <div className="space-y-2">
              ...
              <Input
                ...
                required
              />
              {localState.errors?.quantity && (
                <p className="text-xs text-red-500">{localState.errors.quantity[0]}</p>
              )}
            </div>
            <div className="space-y-2">
              <Label htmlFor="note">Observação / Motivo</Label>
              <Input id="note" name="note" placeholder="Obrigatório para Perda/Retirada" />
              {localState.errors?.note && (
                <p className="text-xs text-red-500">{localState.errors.note[0]}</p>
              )}
            </div>
          </div>
