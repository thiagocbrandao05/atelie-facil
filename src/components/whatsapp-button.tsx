...
export function WhatsAppButton({
  ...
  // Deprecated: não utilizado na nova automação, mantido somente para referência.
  ...
  const openWhatsApp = () => {
    ...
    if (orderStatus === 'QUOTATION') {
      const template =
        templates?.msgQuotation || `Olá {cliente}, aqui está o orçamento dos seus produtos: {link}`
      message = template
      ...
      const template =
        templates?.msgReady ||
        `Olá {cliente}, seu pedido está pronto para retirada! Link do pedido: {link}`
      message = template
        .replace(/\{cliente\}/g, customerName)
        .replace(/\{link\}/g, budgetLink)
  ...
  return (
    ...
    >
      ...
      <span className="text-[10px] font-bold uppercase">Notificar</span>
    </Button>
  )
}
